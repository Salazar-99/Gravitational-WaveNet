...
def generate_data(sample_rate, mass_range, mass_step):
    ...
    Input: sample_rate (Hz) - Integer number of measurements per second
    Input: mass_range (solar masses) - List of form [lower_bound, upper_bound] for mass range
    Input: mass_step (solar masses) - Float indicating step size for mass range
    Output: data - list containing generated waveforms as numpy arrays
    '''
    data = []
...
def save_data(data, sample_rate, mass_range, mass_step):
    ...
    Input: data - output of generate_data()
    Inputs: - The rest are identical to generate_data()
    '''
